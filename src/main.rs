...
fn main() -> Result<(), Error> {
    let acceptable_flags: HashMap<char, &str> = HashMap::from([
        ('h', "--help"),
        ('v', "--version"),
    ]);
    ...
    if contains_flag {
        ...
        for (idx, arg) in args.iter().enumerate() {
            ...
            if !arg.starts_with('-') {
                return Err(Error::new(std::io::ErrorKind::InvalidInput, "Invalid usage. Can't mix flags and arguments."));
            }
            ...
            if is_expanded_flag && acceptable_flags_vals.contains(&&arg.as_ref()) {
                ...
                return Err(Error::new(std::io::ErrorKind::InvalidInput, "Invalid usage. Unknown flag."));
            } else {
